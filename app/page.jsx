    ...
    try {
      ...
      if (json.multiples && json.resultados?.length > 1) {
        ...
        return
            ...
            // ── No encontrado → pedir m² al usuario ─────────────────────────────────
      if (json.noEncontrado || !json.resultados?.length) {
        const newData = { ...d, siiData:{ direccion:`${d.direccion}${d.depto ? ' '+d.depto : ''}, ${d.comuna}` } }
        setData(newData)
        await addAgent(`No encontré esta propiedad en el catastro del SII con esa dirección. Para hacer una tasación precisa necesito los metros cuadrados reales.\n\n¿Cuántos **m² construidos** tiene la propiedad? (superficie total construida, ej: 180)`, 400)
        setInputMode('text')
      ...
      return (
        <div className="sii-bubble">
          ...
          <div className="sii-bubble-grid">
            ...
            {d.m2_construido && <div className="sii-bubble-item"><div className="sii-bubble-label">M² terreno</div><div className="sii-bubble-val">{d.m2_terreno} m²</div></div>}
...
}      if (tasRes.status === 422) {
        // Sin datos SII — no se puede tasar
        const errData = await tasRes.json()
        setMessages(m => m.filter(x => !(x.role==='agent' && x.content?.type==='loading')))
        await addAgent(`Para poder tasar necesito los metros cuadrados reales de la propiedad. No es posible hacer una tasación con datos estimados.\n\n¿Puedes indicarme los **m² construidos** reales? (los encuentras en el certificado de avalúo del SII o en los planos)`, 800)
        setInputMode('text')
        setStage('ingresar_m2_construido')
        return
      }
      if (!tasRes.ok) {useState, useEffect, useRef } from 'react'
