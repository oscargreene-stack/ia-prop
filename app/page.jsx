...
function ChatVendedor({ onBack }) {
  ...
  const iniciarTasacion = async (finalData) => {
    ...
    try {
      ...
      setMessages(m => [...m, { role:'agent', content:{ type:'tasacion', resultado, valorFinal, rangoMin, rangoMax, ajRemo, ajCar, ajJardin, valorBase, siiData: finalData.siiData || null, dirForm: { direccion: finalData.direccion || '', depto: finalData.depto || '', comuna: finalData.comuna || '', tipo: finalData.tipo || '' }, remoInfo:{ tipo: finalData.remodelacion, m2: m2Util, ufM2: AJUSTE_REMO[finalData.remodelacion]||0, tiempo: finalData.tiempo_remo } }}])
    ...
    if (content?.type === 'tasacion') {
      ...
      return (
        <div className="tasacion-card" ref={tasRef}>
          ...
          {/* Informe PDF */}
          <button
            onClick={() => setInforme(content)}
            style={{ marginTop: 14, width: '100%', padding: '11px 14px', borderRadius: 10, border: '1px solid var(--gold)', background: 'var(--gold-dim)', color: 'var(--gold-light)', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}
  ...
  return (
    <div className="chat-app">
      ...
      <div className="messages-area">
        ...
        {informe && <InformePrint data={informe} onClose={() => setInforme(null)} />}
        <div ref={bottomRef}/>
      </div>
...
function InformePrint({ data, onClose }) {
  ...
  return (
    <div id="informe-overlay" style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.65)', overflow: 'auto', padding: '20px 10px' }}>
      ...
      <div className="no-print" style={{ maxWidth: 820, margin: '0 auto 10px', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
        ...
        <button onClick={onClose} style={{ padding: '10px 18px', borderRadius: 8, border: '1px solid #999', background: '#fff', color: '#333', fontSize: 14, cursor: 'pointer' }}>Cerrar</button>
      </div>
